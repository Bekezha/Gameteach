...
// @desc    Get teacher's own games
// @route   GET /api/games/my-games
// @access  Private
export const getMyGames = async (req, res) => {
    try {
        const games = await Game.find({ authorId: req.user._id }).sort({ createdAt: -1 });
        res.status(200).json(games);
    } catch (error) {
        res.status(500).json({ message: "Ошибка сервера при получении ваших игр" });
    }
};
